...
|---|---|
| Baseline | clean SHA tagged and pushed; continuation branch `migration/ros-fsharp-git-provenance` |
...
| F# implementation | MIG-03/04 complete; MIG-06 Git shadow adds typed clean/changed/unavailable status, rename/copy provenance, a process boundary, explicit JSON, and `ros-fs git status` |
...
| Verification | full suite passes: 89 Node, 7 Python, 21 F#, and 7 Node-driven F# differential/smoke tests; solution build has zero warnings/errors; SDE 1.1.1 integrity verified with five pre-existing structural review warnings |
