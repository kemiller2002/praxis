...
Last updated: 2026-09-09 (post `work resume` real-effect slice, Phase A's sixth increment, reusing `work start`'s effect infrastructure verbatim). Production authority remains Node.
...
| Work migration | MIG-07 decision, item/context/backlog-promotion planning, typed evidence-observation, telemetry execution-ID resolution, work-attribution validation (`work validate`, mirroring production `workFindings`), backlog-queue validation (`work backlog-validate`, mirroring production `queueFindings`), and six real effects (`work backlog-transition`, `work capture`, `work update`, `work attach`, `work start`, `work resume`) sub-slices complete — every backlog-only Node command (`add`, `update`, `attach`, `ready`/`block`/`abandon`) now has real F# effect parity, and both `begin` and `resume` (including real telemetry execution creation) are real live-work effects; Node still owns new-execution creation for `complete`, the remaining live-work transitions (`block`/`complete`), and every telemetry-bearing producer command |
| Production command switch | not authorized and not attempted; `DF-ROS-2026-A028` opens the decision track as a three-phase plan (full command-surface effect parity, consumer distribution evidence, then the switch decision itself). Phase A's backlog-layer scope is complete: `work backlog-transition` (`ready`/`block`/`abandon`), `work capture` (`add`/`captureWork`), `work update` (`update`/`findOrCreateQueueEntry`, including its context-only upsert), and `work attach` (`attachFileUnlocked`, including real binary file writes and filename sanitization) are all genuine writes under the same `work-protocol` lock and `backlog-state` recovery journal production's own writer uses. Phase A has also advanced into the live-work layer: `work start` (`startWork`/`transitionUnlocked`'s `begin` action, including real telemetry execution creation via `startExecution`) and `work resume` (`transitionUnlocked`'s `resume` action, reusing the same effect infrastructure) are both real effects under the `work-state` recovery journal. Everything left in Phase A (`block`/`complete`, every telemetry-producer command, both adapter commands) is telemetry- or live-work-entangled; `block`/`complete` can reuse the same effect infrastructure (`block` additionally needs production's own backlog/live-context id-splitting, `complete` needs a new `finalizeWorkExecutions` port), while the telemetry-producer and adapter commands still belong with MIG-08's still-pending scoping decision. `EV-ROS-2026-A043`'s inventory is not yet re-run (its acceptance requires every row) |
...
| Verification | complete gate passes: 104 Node, 7 Python, 178 F#, and 74 differential/smoke tests (363 total); zero-warning build; a direct four-tier compliance sweep (project references, per-file opens, no invented Tier 3 decisions) found no violations |
