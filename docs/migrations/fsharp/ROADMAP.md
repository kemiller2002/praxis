...
| MIG-07 work lifecycle/evidence | in progress | typed live/backlog decisions, pure item/context plans, present/missing/unavailable evidence port, and explicit backlog-to-live promotion plan; exhaustive guards, legal-edge projections, evidence comparisons, ordered multi-item behavior, and all 16 backlog state/action differentials pass; persistence ports exist | state-changing effect execution, evidence-containment policy, and production/distribution switch remain; Node stays state-changing authority |
