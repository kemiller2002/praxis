...
| MIG-07 work lifecycle/evidence | in progress | typed live/backlog decisions, pure item/context/promotion plans, batch evidence composition that runs only after complete semantic acceptance, telemetry execution-ID result-feedback that freezes the item/event projection whenever no new execution record is required, real Git observed/meaningful-path composition for `work context-plan`, read-only `work validate`/`work backlog-validate` diagnostics mirroring production `workFindings`/`queueFindings`, and — Phase A (`DF-ROS-2026-A028`) — four real state-changing effects that give every backlog-only Node command genuine F# parity: `work backlog-transition` (`ready`/`block`/`abandon`), `work capture` (`add`/`captureWork`), `work update` (`update`/`findOrCreateQueueEntry`, including its context-only upsert), and `work attach` (`attachFileUnlocked`, including real binary file writes and production's exact filename sanitization/sequencing). All four commit through the shared `work-protocol` lock and `backlog-state` recovery journal (MIG-05) via `JsonNode` surgery (preserving every unmodeled field, matching production's non-HTML-safe JSON escaping) rather than a full typed queue-item model; see `ARCHITECTURE.md` for the per-slice detail. Exhaustive guards, legal-edge projections, ordered multi-item behavior, all 16 backlog state/action differentials, begin/resume/finalize telemetry differentials, Git differentials, work-attribution/backlog-queue-validation differentials, and effect differentials for all four real effects pass; persistence ports exist | new-execution creation effect, `start`'s live-work/telemetry promotion effect, every live-work transition, evidence-containment policy, and production/distribution switch remain — all telemetry- or live-work-entangled, pending MIG-08's own scoping decision; Node stays authority for every telemetry-bearing and live-work command |
...
Phase 1's backlog-layer scope is now complete: `work backlog-transition`
...
effects — every backlog-only Node command now has genuine F# parity.
Everything still missing from Phase 1 (`start`'s live-work/telemetry
promotion, every live-work transition, every telemetry-producer command,
both adapter commands) is telemetry- or live-work-entangled and belongs
with MIG-08's own scoping decision, not an ad hoc next slice. `EV-ROS-2026-A043`
is not re-run for four rows — its acceptance criterion is every row reading
"full parity" — so the inventory itself is still pending a future re-run
once Phase 1 is materially further along.
