...
| MIG-07 work lifecycle/evidence | in progress | typed decisions, pure item/event/telemetry plan, present/missing/unavailable evidence port, and all-or-nothing multi-item context planning; exhaustive guards, five legal projection edges, evidence differentials, and ordered multi-item begin/rejection comparisons pass; persistence ports exist | state-changing effect execution, backlog promotion, evidence-containment policy, and production/distribution switch remain; Node stays state-changing authority |
