...
| MIG-05 transactional file persistence/recovery | in progress | artifact registries, live event/context, and backlog queue/projection now have separate bounded cross-runtime recovery contracts; production backlog read/modify/write is serialized under `work-protocol` and parallel-capture proof passes; telemetry recovery remains | retain stateful Node writers; each semantic store keeps a bounded transaction contract |
...
1. Finish MIG-05 with bounded telemetry recovery semantics; do not enlarge the
   three established store contracts into a generic transaction framework.
