...
| MIG-07 work lifecycle/evidence | in progress | typed live/backlog decisions, pure item/context/promotion plans, batch evidence composition that runs only after complete semantic acceptance, telemetry execution-ID result-feedback that freezes the item/event projection whenever no new execution record is required, real Git observed/meaningful-path composition (config-driven glob filter plus optional `ROS_BASE_REF` committed-range diff) for `work context-plan`, a read-only `work validate` diagnostic mirroring production `workFindings` (enforcement gate, baseline/attribution/active-work exclusions, and the synthetic `.git` finding on Git failure), a read-only `work backlog-validate` diagnostic mirroring production `queueFindings` (duplicate/invalid id, invalid status, invalid priority over the raw `.ros/work/queue.json` rows), and — the first Phase A (`DF-ROS-2026-A028`) real state-changing effect — `work backlog-transition`, a genuine write to `.ros/work/queue.json`/`queue.md` for the backlog-only `ready`/`block`/`abandon` actions (mirroring production `backlogTransition`/`backlogTransitionUnlocked`), guarded by the shared `work-protocol` file lock and the `backlog-state` recovery journal (MIG-05) and proven byte-identical to production's own write (including untouched items/fields and non-HTML-safe JSON escaping) via `JsonNode` surgery rather than a full typed queue-item model, plus a second real effect — `work capture`, mirroring production `add`/`captureWork` (title/priority validation, explicit-id validation against both the queue and the live context, collision-avoiding sequential ID generation over `nextSeq`, and the same default document production's `loadQueue` synthesizes when `queue.json` does not exist yet), excluding `--file` attachment, plus a third real effect — `work update`, mirroring production `update`/`findOrCreateQueueEntry` (per-field `Keep`/`Set` changes only for fields explicitly provided, and the same context-only-id upsert with production's exact minimal-record defaults), also excluding `--file` attachment; exhaustive guards, legal-edge projections, evidence comparisons, ordered multi-item behavior, all 16 backlog state/action differentials, begin/resume/finalize telemetry differentials, Git baseline/meaningful-path/base-ref differentials, work-attribution differentials, backlog-queue-validation differentials, and effect differentials for all three real effects pass; persistence ports exist | new-execution creation effect, `start`'s live-work/telemetry promotion effect, `--file` attachment, evidence-containment policy, and production/distribution switch remain; Node stays authority for every telemetry-bearing and live-work command |
...
1. **Full command-surface effect parity** — real, persisted F# handlers
   ...
   plus new modeling for backlog `--file` attachment and telemetry producer
   commands the roadmap has not yet named as their own slice.
...
Phase 1's first three increments landed: `work backlog-transition` gives
`ready`/`block`/`abandon` a real, differential-proven effect; `work capture`
gives production's `add` the same treatment; `work update` gives production's
`update` the same treatment, including its context-only-id upsert (all three
excluding `--file` attachment). `EV-ROS-2026-A043` is not re-run for three
rows — its acceptance criterion is every row reading "full parity" — so the
inventory itself is still pending a future re-run once Phase 1 is materially
further along.
