...
Phase 1's first increment landed: `work backlog-transition` gives
`ready`/`block`/`abandon` a real, differential-proven effect. `EV-ROS-2026-A043`
is not re-run for one row — its acceptance criterion is every row reading
"full parity" — so the inventory itself is still pending a future re-run once
Phase 1 is materially further along.
