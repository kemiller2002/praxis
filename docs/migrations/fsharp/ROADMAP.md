...
| MIG-07 work lifecycle/evidence | in progress | typed live/backlog decisions, pure item/context/promotion plans, batch evidence composition that runs only after complete semantic acceptance, telemetry execution-ID result-feedback that freezes the item/event projection whenever no new execution record is required, and real Git observed/meaningful-path composition (config-driven glob filter plus optional `ROS_BASE_REF` committed-range diff) for `work context-plan`; exhaustive guards, legal-edge projections, evidence comparisons, ordered multi-item behavior, all 16 backlog state/action differentials, begin/resume/finalize telemetry differentials, and Git baseline/meaningful-path/base-ref differentials pass; persistence ports exist | new-execution creation effect, state-changing effect execution, evidence-containment policy, and production/distribution switch remain; Node stays state-changing authority |
