...
retention policy, and provenance-source dedup. Everything still missing
from Phase 1 (every remaining write-path telemetry-producer command,
both adapter commands, every non-generic adapter's field mapping, and
`resume`'s still-open `parentExecutionId` linkage) remains its own
...
`telemetry summary`, `telemetry finalize`, `telemetry record`, and
`telemetry ingest` increments above each flip one more of the still-open
rows to "full parity" (seventeen of twenty-six), not yet reflected in a
new formal re-run, since a small row delta does not warrant its own
evidence record -- the acceptance criterion is still every row reading
"full parity," so Phase 1 remains open until a future re-run closes the
rest.
