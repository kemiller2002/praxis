...
`complete`) now has genuine F# parity. Everything still missing from Phase 1
(every telemetry-producer command, both adapter commands) is MIG-08-sized
scoping work, independent of the now-complete work-lifecycle command
surface. `EV-ROS-2026-A043` is not re-run for eight rows — its acceptance
