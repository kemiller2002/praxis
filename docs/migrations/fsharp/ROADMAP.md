...
| MIG-07 work lifecycle/evidence | in progress | typed live/backlog decisions, pure item/context/promotion plans, and batch evidence composition that runs only after complete semantic acceptance; exhaustive guards, legal-edge projections, evidence comparisons, ordered multi-item behavior, and all 16 backlog state/action differentials pass; persistence ports exist | telemetry-result/event finalization, state-changing effect execution, evidence-containment policy, and production/distribution switch remain; Node stays state-changing authority |
