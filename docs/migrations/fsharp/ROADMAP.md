...
write-path, lock, identity, or adapter complexity. Everything still missing
from Phase 1 (`telemetry summary`'s aggregation, every write-path
telemetry-producer command, both adapter commands) remains its own future
MIG-08 scoping choice. `EV-ROS-2026-A043` is not re-run for nine rows — its
