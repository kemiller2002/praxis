...
1. **Full command-surface effect parity** — real, persisted F# handlers
   ...
   plus new modeling for backlog update/attach and telemetry producer
...
Phase 1's first two increments landed: `work backlog-transition` gives
`ready`/`block`/`abandon` a real, differential-proven effect, and `work
capture` gives production's `add` the same treatment (excluding `--file`
attachment). `EV-ROS-2026-A043` is not re-run for two rows — its acceptance
criterion is every row reading "full parity" — so the inventory itself is
still pending a future re-run once Phase 1 is materially further along.
