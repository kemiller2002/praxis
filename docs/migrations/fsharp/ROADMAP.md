...
| MIG-07 work lifecycle/evidence | in progress | typed decisions plus pure item/event/telemetry orchestration plan; exhaustive transition/rejection tests and all five legal item/event projection differentials pass; persistence ports exist | evidence-path capability, whole-context/multi-item orchestration, backlog promotion, and production/distribution switch remain; Node stays state-changing authority |
