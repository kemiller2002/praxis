...
| MIG-07 work lifecycle/evidence | in progress | typed decisions, pure item/event/telemetry plan, and present/missing/unavailable evidence port; exhaustive guards, five legal projection edges, and file/directory/absolute/missing differentials pass; persistence ports exist | whole-context/multi-item orchestration, backlog promotion, evidence-containment policy, and production/distribution switch remain; Node stays state-changing authority |
