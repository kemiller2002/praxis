...
| execution actor | the execution record's `actor` (`praxis.actor/1`) projects that identity plus `ROS_ACTOR_KIND`/`ROS_ACTOR` into the canonical actor every event, backlog item, and artifact contribution of the run references by `executionId` (see `docs/agent-identity-and-provenance.md`) |
