...
export async function main(argv) {
  try {
    ...
    if (command === "init") {
      ...
      if (!result.dryRun) {
        if (!/^\d+\.\d+\.\d+$/.test(result.packageVersion)) {
          console.log(
            `warning: ${result.packageVersion} is a main-branch snapshot, not a stable release -- ` +
              "./ros needs the matching self-contained binary from a GitHub Release, and none is " +
              "ever published for a snapshot version. Install a stable published version instead " +
              "to get a working ./ros."
          );
        }
