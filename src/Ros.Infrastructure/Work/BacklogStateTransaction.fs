...
module BacklogStateTransaction =
    let private resource = "backlog-state"
    let private schemaVersion = "1.0.0"

    let private expectedPaths =
        [ ".ros/work/queue.json"; ".ros/work/queue.md" ]
    ...
    let private validHash (value: string) =
        value.Length = 64 && value |> Seq.forall (fun character -> Char.IsDigit character || character >= 'a' && character <= 'f')
    ...
    let private writeAtomic (file: string) (content: string) =
        ...
        with error ->
            ...
            Error(failure "write backlog state" None WorkPersistenceOutcome.Indeterminate error.Message)
    ...
    let private parse file =
        try
            ...
            if stringProperty "schemaVersion" <> Some schemaVersion || stringProperty "resource" <> Some resource then
                Error(failure "recover backlog-state transaction" None WorkPersistenceOutcome.Indeterminate "unsupported transaction record")
            elif not (validPaths (writes |> List.map _.Path))
                 || writes
                    |> List.exists (fun write ->
                        not (validHash write.AfterSha256)
                        || write.BeforeSha256 |> Option.exists (validHash >> not)
                        || hashText write.Content <> write.AfterSha256) then
                Error(failure "recover backlog-state transaction" None WorkPersistenceOutcome.Indeterminate "transaction write set is invalid")
            else
                Ok writes
        with error ->
            Error(failure "recover backlog-state transaction" None WorkPersistenceOutcome.Indeterminate error.Message)

    let private prepareUnsafe (root: string) (writes: BacklogStateWrite list) =
        let file = transactionPath root

        if not (validPaths (writes |> List.map _.Path)) then
            Error(failure "prepare backlog-state transaction" None WorkPersistenceOutcome.Failed "transaction must contain queue then projection")
        elif File.Exists file then
            Error(failure "prepare backlog-state transaction" None WorkPersistenceOutcome.Failed "a pending backlog-state transaction already exists")
        else
            let pending =
                writes
                |> List.map (fun write ->
    ...
    let prepare (root: string) (writes: BacklogStateWrite list) =
        try
            prepareUnsafe root writes
        with error ->
            Error(failure "prepare backlog-state transaction" None WorkPersistenceOutcome.Failed error.Message)
        ...
        if not (File.Exists file) then
            ...
            | Ok writes ->
                ...
                | Some write ->
                    Error(
                        failure
                            "recover backlog-state transaction"
                            (Some write.Path)
                    ...
                    let rec replay remaining =
                        ...
                        | [] ->
                            try
                                ...
                                Ok()
                            with error ->
                                Error(failure "complete backlog-state transaction" None WorkPersistenceOutcome.Indeterminate error.Message)
                        | write :: rest ->
