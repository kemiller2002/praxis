            ...
            Run =
              fun () ->
                  withTemporaryRoot (fun root ->
                      ...
                      match FileWorkContextRepository.applyContextPlan root "repo" Actor.unknown plan with
          ...
          { Name = "re-applying the same event twice does not duplicate the events.jsonl line, matching production's own eventId dedup"
            Run =
              fun () ->
                  withTemporaryRoot (fun root ->
                      let plan = planFor (beginItem "WI-NEW" [ "EXE-1" ])
                      FileWorkContextRepository.applyContextPlan root "repo" Actor.unknown plan |> ignore
                      FileWorkContextRepository.applyContextPlan root "repo" Actor.unknown plan |> ignore
            ...
            Run =
              fun () ->
                  withTemporaryRoot (fun root ->
                      ...
                      FileWorkContextRepository.applyContextPlan root "repo" Actor.unknown beginPlan |> ignore
                      ...
                      match FileWorkContextRepository.applyContextPlan root "repo" Actor.unknown blockPlan with
                      | Error message -> failwith message
                      | Ok _ ->
                          let raw = File.ReadAllText(Path.Combine(root, ".ros", "context", "current.json"))
